...
```
  Skipping due to the dependency hell we enter when trying tinstall rgdal

  # Longitude and Latitude
  library(rgdal) # read the shapefile with readOGR()
  shp <- readOGR(dsn = "../data/moscow_adm.shp", layer = "moscow_adm")
...
## The following code was added to this script by the authors of this repo
```{r}
...
```{r}
# output .csv dataset with newly engineered features
write.csv(train, "../data/cleanEngineeredTrain.csv")
write.csv(train[sample(nrow(train), 500),], "../data/sampleCleanEngineeredTrain.csv")
```
