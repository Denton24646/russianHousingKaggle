...
library(mice)
imp.macro_vars <- mice(macro_vars, m=1, method='cart', maxit = 3, printFlag=TRUE)

#??mice
# check performance 
summary(imp.macro_vars)

imp.macro_vars$imp$balance_trade

completedmacroData <- complete(imp.macro_vars,1)

summary(completedmacroData)

write.csv(completedmacroData, "complete_macro_data.csv")
